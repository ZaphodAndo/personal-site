import { getFilePath } from "../utils/getFilePath.js";

function constructPost(post) {
    return `
        <a href="${post.url}" class="post">
            <p>${post.data.title}</p>
            <time datetime="${post.date.toISOString()}">
                ${post.date.toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric" })}
            </time>
        </a>
    `;
}

export const data = {
    layout: "base.11ty.js",
    title: "Ethan Anderson - Home",
    pageCSS: [getFilePath("../css/views/index.css")],
};

export function render(data) {
    return `
        <h2>Latest Posts</h2>
        <div class="latest-posts">
            ${data.collections.post
                .sort((a, b) => b.date - a.date)
                .slice(0, 3)
                .map((post) => constructPost(post))
                .join("\n")}
        </div>

        <h2>Projects</h2>
        <ul>
            <li><a href="https://github.com/ZaphodAndo/hacker-news" target="_blank">hacker-news</a></li>
            <li><a href="https://github.com/ZaphodAndo/linqem" target="_blank">linqem</a></li>
            <li><a href="https://github.com/ZaphodAndo/scribble" target="_blank">scribble</a></li>
            <li><a href="https://github.com/ZaphodAndo/scuffed-text-ui" target="_blank">scuffed-text-ui</a></li>
        </ul>

        <h2>"Fun" Zone</h2>
        <div class="fun-zone">
            <button type="button"><svg><use href="public/icons.svg#rain-icon" /></svg> start rain</button>
            <button type="button"><svg><use href="public/icons.svg#firework-icon" /></svg> fireworks</button>
        </div>
    `;
}
